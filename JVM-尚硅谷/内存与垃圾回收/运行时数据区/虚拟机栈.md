...
1. 在栈顶栈帧中调用一个对象的方法的时候首先会将对象压入操作数栈，然后执行不同的调用方法，如果是虚方法，那字节码指令是invokevirtual，根据invokevirtual找到执行的*实际方法*，记作C。
...
```
PC寄存器存储的是该方法要执行的下一条指令的值，那么我们将PC寄存器里保存的下一条指令的值存放到方法返回地址这个结构里。就是交给执行引擎让其继续接着执行后续的操作。
```
